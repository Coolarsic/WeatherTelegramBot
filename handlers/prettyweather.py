...
@dp.message_handler(text='\U0001F326 Погода на неделю')
async def pretty_weather(message: types.Message):
    try:
        ...
        if coords == (None, None):
            raise ValueError
        somemedia = InputFile('data/photos/second_please.jpg')
        sec_message = await bot.send_photo(message.from_user.id, photo=somemedia, caption='<b>Секундочку...</b>', parse_mode=ParseMode.HTML)

        im = Image.new('RGBA', WINDOW_SIZE, color=('white'))  # #202124
        font_main = ImageFont.truetype('data/fonts/19755.ttf', size=25)
        font_day = ImageFont.truetype('data/fonts/19755.ttf', size=20)
        font_simple = ImageFont.truetype('data/fonts/19755.ttf', size=18)
        ...
        im.save(f'{trash_number}.png', 'PNG')
        file_to_send = InputFile(f'{trash_number}.png')
        ...
        session.commit()
        session.close()
        os.remove(f'{trash_number}.png')
    except Exception:
        await bot.send_message(message.from_user.id, "Извините, что-то пошло не так. Проверьте правильность написания города или попробуйте "
                                                     "попытку позже" + SAD_ART)
